...
Compare the above to what the code was previously. Note how much mor streamlined the new code is.
...
Note that by design, _if_ the directory exists, the `if` statement doesn't trigger, so nothing happens. It is only if the directory _does not_ exist, that something happens. We don't need to specify that nothing happens if the directory _does_ exist.
...
- `thenrm $output`: "...then remove the output file"
...
- If it was not run with sudo, exit the script with a message that tells the user to run the script using `sudo`.
</summary>
...
- `if [ $UID -eq 0 ]` "If $UID equals zero..."
